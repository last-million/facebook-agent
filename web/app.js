...
function uxAttachNotGroupMemberProfiles() {
  const list = $("notGroupMemberProfilesList");
  if (!list) return;
  async function load() {
    try {
      const data = await api("/api/profiles/not-group-member");
      const rows = (data && data.notGroupMember) || [];
      const c = $("notGroupMemberProfilesCount"); if (c) c.textContent = String(rows.length);
      if (!rows.length) { list.innerHTML = '<div style="opacity:.6">No profiles excluded from every group — all working somewhere. ✓</div>'; return; }
      list.innerHTML = "";
      for (const r of rows) {
        const row = document.createElement("div"); row.className = "discRow";
        const info = document.createElement("div"); info.className = "di-info";
        info.innerHTML = '<span class="di-id">Profile ' + (r.profileId || "?") + '</span>' + (r.label ? ' <span class="di-meta">' + r.label + '</span>' : '') + '<div class="di-meta">' + (r.reason || "not a member of any configured group") + (r.at ? ' · ' + r.at : '') + '</div>';
        const btn = document.createElement("button"); btn.type = "button"; btn.textContent = "Release";
        btn.title = "Join this account to a group on Facebook first, then Release to let the agent try it again everywhere";
        btn.addEventListener("click", async () => { btn.disabled = true; try { await api("/api/profiles/release-not-group-member?profileId=" + encodeURIComponent(r.profileId), { method: "POST", body: "{}" }); await load(); } catch (e) { btn.disabled = false; if (typeof showToast === "function") showToast("Release failed"); } });
        row.appendChild(info); row.appendChild(btn); list.appendChild(row);
      }
    } catch (e) { list.innerHTML = '<div style="opacity:.6">Could not load profiles not member of any group.</div>'; }
  }
  $("refreshNotGroupMemberProfilesBtn")?.addEventListener("click", () => load());
  load();
}
...
(function bootUxOverhaul() {
  const run = () => {
    ...
    try { uxAttachNotGroupMemberProfiles(); } catch (err) { console.warn("uxAttachNotGroupMemberProfiles failed", err); }
