...
(async()=>{
  const profileId = 16;
  const postUrl = 'https://www.facebook.com/groups/4854972804605257/permalink/25916569904685574/';
  ...
  try {
    ...
    const snap = await page.evaluate(()=>{
      ...
      return {
        ...
        hasSylikes: body.includes('sylikes.com'),
        mavlynkUrls: [...new Set((body.match(/mavlynk\.com\/[A-Za-z0-9]+/g) || []))],
        sylikesUrls: [...new Set((body.match(/sylikes\.com\/[A-Za-z0-9]+/g) || []))],
        commentSnippets: body.split('\n').filter(l => /mavlynk|sylikes|deal|Save With Emily/i.test(l)).slice(0,8),
      };
    });
