...
4. **(2026-06-17) ARRAY-CLOBBER: a dashboard-added posting group "saved then vanished after restart."** `deepMerge` (server.js ~1728) REPLACES arrays wholesale (`result[key]=value` for array values — no element merge), and the fix-#1 control-flag guard (~990) only preserves operator FLAGS, NOT `posting.groups`. So any BACKGROUND full-state write holding a STALE snapshot — the autopilot tick (writeState(state,{controlWrite:true}) ~9513/9527, a minutes-long read→post→write window), the 24/7 harvest cursor write, the open-budget persist — silently reverted `posting.groups` to its pre-edit array, dropping the group the operator just added. `controlWrite:true` (which those background writes use) BYPASSED the flag guard too. PROVEN via git history of the auto-synced state: the operator's added group `1322293839675416` never appeared in any committed version while only `4854972804605257` persisted. **FIXED (commit b7541a5)**: writeState now preserves `posting.groups`+`groupUrls` from the FRESH on-disk `existing` on EVERY write EXCEPT `opts.allowOperatorConfig` (set ONLY by the dashboard's `PUT /api/state` ~19459 — the sole legit group-editor; all dashboard edits route collectState→PUT per app.js ~4522). VERIFIED: group survives a restart AND a background state-write. NOTE: posting.groups has NO other writer (no direct `posting.groups=` assignment anywhere), so gating on the PUT is complete. If a NEW endpoint ever needs to edit groups, it MUST pass `allowOperatorConfig:true`. Same array-clobber class could affect other operator-edited arrays (sourceUrls, profile lists) — extend the preserve-list if they report loss.
